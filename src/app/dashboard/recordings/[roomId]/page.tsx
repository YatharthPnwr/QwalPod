      ...
      const interval = setInterval(() => {
        if (!meetingURLS) {
          const getMeetingURLS = async () => {
            ...
            const keys = Object.keys(res.data.getAllFilesResponse);
            let allFilesReceived = true;
            console.log("The keys are", keys);
            keys.forEach((user) => {
              console.log("user is", user);
              if (
                (res.data.getAllFilesResponse[user].audio.length > 0 &&
                  res.data.getAllFilesResponse[user].video.length == 0) ||
                (res.data.getAllFilesResponse[user].audio.length > 0 &&
                  res.data.getAllFilesResponse[user].video.length == 0)
              ) {
            ...
            if (allFilesReceived) {
              console.log("All files are received, setting the status to true");
    ...
    <div className="min-h-screen bg-background">
      ...
      <SignedIn>
        ...
        <main className="container mx-auto px-4 md:px-6 py-8">
          ...
          {!loaded && (
            <div className="flex flex-col items-center justify-center py-20 gap-4">
              ...
              <p className="text-muted-foreground">
                meeting assets will automatically appear here
              </p>
            </div>
          )}
          ...
          {/* Empty State */}
          {loaded &&
            (!meetingURLS || Object.keys(meetingURLS).length === 0) && (
              <Card className="flex flex-col items-center justify-center py-20 px-4 text-center">
                ...
                <p className="text-muted-foreground text-sm max-w-sm">
                  There are no recordings available for this session yet. Come
                  back later.
                </p>
              </Card>
            )}
